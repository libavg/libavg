...
.. automodule:: libavg.avg
    ...
    .. autoclass:: Logger
        ...
        A commonly shared logger for libavg applications.
        The logger supports custom log sinks, categorized log messages and log
        severities.
        An instance of the logger is available in the libavg module:
       ...
       **Severities**
        ...
        .. py:method:: setCategories(categories)
            ...
            This can also be achieved using :envvar:`AVG_LOG_CATEGORIES`.
            with the format:
            ...
            where terms in brackets are optional.

            Thus it is possible to set a severity level for every category. If no severity is
            given, the default severity is used. (refer to: :py:meth:`setDefaultSeverity`)
            
            .. code-block:: bash

                export AVG_LOG_CATEGORIES="CONFIG:INFO NONE PROFILE:DEBUG MEMORY"

            Default categories are :py:const:`NONE`, :py:const:`APP` and
            :py:const:`DEPRECATION`. They are set to the defaultSeverity.
