...
class MTemu(object):
    ...
    def __init__(self):
        ...
        self.__rootNode.connectEventHandler(avg.CURSORUP, avg.MOUSE, self,
                self.__onMouseUp)
        self.__rootNode.connectEventHandler(avg.CURSORDOWN, avg.MOUSE, self,
                self.__onMouseDown)
        self.__rootNode.connectEventHandler(avg.CURSORMOTION, avg.MOUSE, self,
                self.__onMouseMotion)
        posX = self.__rootNode.size.x * 2/5
        posY = self.__rootNode.size.y-40

        self.__layer = avg.WordsNode(text='Multitouch emulation active', pos=(posX, posY),
                color='DDDDDD', sensitive=False, fontsize=20, parent=self.__rootNode)
    ...
    def __onMouseDown(self, event):
        if self.mouseState == 'Up' and event.button == 1:
            ...
            if self.dualTouch and not self.secondTouch:
                self._sendFakeTouch(self.cursorID+1, event.pos, event.type, True)
    ...
    def toggleDualTouch(self):
        ...
        if self.mouseState == 'Down':
            if self.secondTouch:
                ...
                self._sendFakeTouch(self.cursorID+1, Point2D(0,0), avg.CURSORDOWN,
                        mirror=True)
    ...
    def _sendFakeTouch(self, cursorID, pos, touchType, mirror=False):
        ...
        g_Player.getTestHelper().fakeTouchEvent(cursorID,touchType, self.source, pos,
                Point2D(0,0))

    def _releaseTouch(self, cursorID):
       self._sendFakeTouch(cursorID, Point2D(0,0), avg.CURSORUP)
