...
TrapezoidAndBarrel::TrapezoidAndBarrel(IntRect srcRect, double K1, double T, double RescaleFactor)
    : m_TrapezoidFactor(T),
      m_K1(K1),
...
TrapezoidAndBarrel::~TrapezoidAndBarrel()
{
}

DPoint TrapezoidAndBarrel::inv_distortion(const DPoint &pt)
{
    if (fabs(m_K1)<1e-10){
        return DPoint(pt);
    }
...
DPoint TrapezoidAndBarrel::distortion(const DPoint &pt)
{
    DPoint pt_norm = (pt - m_Center)/m_Scale;
...
DPoint TrapezoidAndBarrel::inv_trapezoid(const DPoint &pt)
{
    //stretch x coord
...
DPoint TrapezoidAndBarrel::trapezoid(const DPoint &pt)
{
    //stretch x coord
    double yn = (pt.y - m_Center.y)/m_TrapezoidScale;
    return DPoint( 
...
DPoint TrapezoidAndBarrel::inverse_transform_point(const DPoint &pt)
{
    return inv_trapezoid(inv_distortion(pt));
}

DPoint TrapezoidAndBarrel::transform_point(const DPoint &pt)
{
    return distortion(trapezoid(pt));
}

double TrapezoidAndBarrel::getPixelSize(const DPoint &pt)
{
