        ...
        float frameRate)
{
    ...
    try {
        if (sDriver == "firewire") {
            ...
            AVG_TRACE(logging::subsystem::WARNING, "Firewire camera specified, but firewire "
            ...
            pCamera = CameraPtr(new V4LCamera(sDevice, unit, captureSize, camPF, 
                    destPF, frameRate));
#else
            AVG_TRACE(logging::subsystem::WARNING, "Video4Linux camera specified, but "
            ...
            pCamera = CameraPtr(new DSCamera(sDevice, captureSize, camPF, destPF,
                frameRate));
#else
            AVG_TRACE(logging::subsystem::WARNING, "DirectShow camera specified, but "
