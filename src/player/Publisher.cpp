...
Publisher::Publisher() { m_pPublisherDef = PublisherDefinition::create(""); }

Publisher::Publisher(const string& sTypeName) {
    m_pPublisherDef =
        PublisherDefinitionRegistry::get()->getDefinition(sTypeName);
...
int Publisher::subscribe(MessageID messageID, PyObject* pCallable) {
    if (PyCallable_Check(pCallable)) {
        ...
        s_LastSubscriberID++;
        //        cerr << this << " subscribe " << messageID << ", " <<
        //        subscriberID << endl;
        subscribers.push_front(
            SubscriberInfoPtr(new SubscriberInfo(subscriberID, pCallable)));
        return subscriberID;
    } else {
...
void Publisher::unsubscribe(MessageID messageID, int subscriberID) {
    //    cerr << this << " unsubscribe " << messageID << ", " << subscriberID
    //    << endl;
    //    cerr << "  ";
...
void Publisher::unsubscribeCallable(MessageID messageID, PyObject* pCallable) {
    ...
    if (numSubscribers > 1) {
        throw Exception(
            AVG_ERR_INVALID_ARGS,
            "Signal with ID " + toString(messageID) +
...
void Publisher::publish(MessageID messageID) {
    if (m_SignalMap.find(messageID) != m_SignalMap.end()) {
        throw Exception(
            AVG_ERR_INVALID_ARGS,
            "Signal with ID " + toString(messageID) + "already registered.");
    }
...
void Publisher::notifySubscribersPy(MessageID messageID, const py::list& args) {
    ...
    for (SubscriberInfoList::iterator it = subscribers.begin();
...
void Publisher::unsubscribeIterator(MessageID messageID,
                                    SubscriberInfoList::iterator it) {
    m_SignalMap[messageID].erase(it);
}

Publisher::SubscriberInfoList& Publisher::safeFindSubscribers(
    MessageID messageID) {
    if (m_SignalMap.find(messageID) == m_SignalMap.end()) {
        throw Exception(AVG_ERR_INVALID_ARGS,
                        "No signal with ID " + toString(messageID));
    }
...
void Publisher::throwSubscriberNotFound(MessageID messageID, int subscriberID) {
    if (subscriberID == -1) {
        ...
        throw Exception(AVG_ERR_INVALID_ARGS,
                        "Signal with ID " + toString(messageID) +
                            " doesn't have a subscriber with ID " +
                            toString(subscriberID));
    }
}
