...
class SpeedDiv(app.MainDiv):
    def onArgvParserCreated(self, parser):
        ...
        parser.add_option('--num-objs', '-n', dest='numObjs',
                type='int', default=40,
                help='number of curves to create [Default: 40]')
        parser.add_option('--radius', '-r', dest='radius',
                type='int', default=100,
                help='Size of the curves [Default: 100]')
    ...
    def __createNodes(self):
        ...
        for i in xrange(self.__optNumObjs):
            pos = avg.Point2D(random.randrange(800-64), random.randrange(600-64))
            pos1 = pos + (random.randrange(self.__optRadius)-self.__optRadius/2,
                    random.randrange(self.__optRadius)-self.__optRadius/2)
            pos2 = pos + (random.randrange(self.__optRadius)-self.__optRadius/2,
                    random.randrange(self.__optRadius)-self.__optRadius/2)
            pos3 = pos + (random.randrange(self.__optRadius)-self.__optRadius/2,
                    random.randrange(self.__optRadius)-self.__optRadius/2)
            node = avg.CurveNode(parent=self, pos1=pos, pos2=pos1, pos3=pos2, pos4=pos3,
                    color="FFFFFF")
